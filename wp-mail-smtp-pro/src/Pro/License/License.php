...
class License {
	...
	public function verify_key( $key = '', $ajax = false ) {
		...
		$all_opt['license']['type']        = isset( $verify->type ) ? $verify->type : $all_opt['license']['type'];
	...
	public function maybe_validate_key() {
		...
		if ( empty( $all_opt['license']['updates'] ) ) {
			$all_opt['license']['updates'] = strtotime( '+24 hours' );
			$options->set( $all_opt );
	...
	public function validate_key( $key = '', $forced = false, $ajax = false ) {
		...
		if ( isset( $validate->key ) || isset( $validate->author ) ) {
			$all_opt['license']['is_expired']  = false;
			$all_opt['license']['is_disabled'] = false;
			$all_opt['license']['is_invalid']  = true;
			$options->set( $all_opt );
			if ( $ajax ) {
		...
		if ( isset( $validate->expired ) ) {
			$all_opt['license']['is_expired']  = true;
			$all_opt['license']['is_disabled'] = false;
			$all_opt['license']['is_invalid']  = false;
			$options->set( $all_opt );
			if ( $ajax ) {
		...
		if ( isset( $validate->disabled ) ) {
			$all_opt['license']['is_expired']  = false;
			$all_opt['license']['is_disabled'] = true;
			$all_opt['license']['is_invalid']  = false;
			$options->set( $all_opt );
			if ( $ajax ) {
		...
		$all_opt['license']['type']        = isset( $validate->type ) ? $validate->type : $all_opt['license']['type'];
		$all_opt['license']['is_expired']  = false;
		$all_opt['license']['is_disabled'] = false;
		$all_opt['license']['is_invalid']  = false;
		$options->set( $all_opt );
